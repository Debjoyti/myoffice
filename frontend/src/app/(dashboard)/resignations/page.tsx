...
import { LogOut, Calculator, Clock, CheckCircle2, Plus, FlaskConical } from 'lucide-react'
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <Card padding="none">
        ...
        {filtered.length === 0 ? (
          ...
          <Table>
            <Thead><tr><Th>Employee</Th><Th>Designation</Th><Th>Submitted</Th><Th>Last Working Day</Th><Th>Notice Period</Th><Th>Settlement</Th><Th>Status</Th></tr></Thead>
            <Tbody>
              {filtered.map(r => (
      ...
      {/* Detail Modal */}
      <Modal open={!!selected} onClose={() => setSelected(null)} title={`Resignation — ${selected?.employee}`} size="md"
        footer={<>
          <Button variant="ghost" size="sm" onClick={() => setSelected(null)}>Close</Button>
          {selected?.status === 'Submitted' && <Button size="sm">Acknowledge & Start Notice</Button>}
          {selected?.status === 'Under Notice' && <Button size="sm">Process Relieving</Button>}
        </>}
      >
        {selected && (
