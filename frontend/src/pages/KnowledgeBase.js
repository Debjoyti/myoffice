      ...
      <div className="page-content">
        <div className="page-inner">
          ...
          <div style={{ position: 'relative', marginBottom: '32px' }}>
            <Search style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', color: 'rgba(255,255,255,0.4)' }} size={20} />
            <input 
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
              {filteredArticles.map(article => (
                <div key={article.id} className="dark-card" style={{ padding: '24px', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                    ...
                    <span style={{ fontSize: '11px', fontWeight: 700, background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.5)', padding: '4px 10px', borderRadius: '50px' }}>
                      {article.category.toUpperCase()}
                    </span>
                  </div>
                  <h3 style={{ color: '#fff', fontSize: '17px', margin: '0 0 8px', fontWeight: 600 }}>{article.title}</h3>
                  <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '14px', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{article.content}</p>
                  <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.3)' }}>By {article.author_name}</span>
        ...
        <div className="dark-modal-overlay">
          <div className="dark-modal" style={{ maxWidth: '800px' }}>
            <div className="dark-modal-header">
              <h2 style={{ color: '#fff', margin: 0 }}>New Article</h2>
